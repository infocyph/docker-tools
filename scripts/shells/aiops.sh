...
aiops_generate_document_patch() {
  ...
  retry_system="$system"
  retry_system+=  local file="$1" context="$2" request="$3" extra_system="$4" context_only="$5" think_override="${6:-inherit}"
...
main "$@"
\nThe previous response failed strict JSON validation. Return only the required JSON object with no prose, Markdown fences, or trailing text.'
