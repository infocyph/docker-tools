...
--pick)            want_pick=1; shift ;;
  --app)             want_field="app"; shift ;;
  --container)       want_field="container"; shift ;;
  --profile)         want_field="profile"; shift ;;
  --docroot|--dir)   want_field="docroot"; shift ;;
  --quiet)           quiet=1; shift ;;
  --list)            mode="list"; shift ;;
  --list-domains)    mode="list-domains"; shift ;;
  --list-directories|--list-dirs) mode="list-directories"; shift ;;
  --list-containers) mode="list-containers"; shift ;;
...
emit_single_from_domain() {
  ...
  case "$app" in
  php)  container="$(printf '%s\n' "$meta" | kv_get php_container)"; profile="$(printf '%s\n' "$meta" | kv_get php_profile)" ;;
  node) container="$(printf '%s\n' "$meta" | kv_get node_container)"; profile="$(printf '%s\n' "$meta" | kv_get node_profile)" ;;
  *)    container=""; profile="" ;;
  esac
...
domains_matching_docroot() {
  local want; want="$(normalize_docroot "$1")"
  [[ -n "$want" ]] || return 0
  local d meta dr
...
pick_one_domain() {
  ...
  while true; do
    ...
    (( ans>=1 && ans<=${#arr[@]} )) || { echo "Out of range." >&2; continue; }
    echo "${arr[$((ans-1))]}"
    return 0
  done
}
